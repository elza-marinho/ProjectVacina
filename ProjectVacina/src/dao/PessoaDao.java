...
public class PessoaDao {
	public Pessoa cadastrar(Pessoa novaPessoa) {
		...
		try {
			...
			stmt.setInt(4, novaPessoa.getIdade());
	...
	public boolean alterar(Pessoa pessoa) {
		...
		String sql = "UPDATE PESSOA " + "SET NOME=?, CPF=?, IDADE=?, SEXO=?";
	...
	public boolean cpfJaUtilizado(String cpf) {
		...
		Connection conn = Banco.getConnection();
		String sql = " select id from pessoa p " + "where p.cpf = '" + cpf + "'";
		PreparedStatement stmt = Banco.getPreparedStatement(conn, sql);
		ResultSet rs = null;
	...
	public Pessoa pesquisarPorIdade(int idade) {

		String sql = "SELECT * FROM PESSOA WHERE IDADE=?";
		Pessoa pessoaPesquisada = null;
		try (Connection conn = Banco.getConnection();
			PreparedStatement consulta = Banco.getPreparedStatement(conn, sql);) {
			consulta.setInt(3, idade);
			ResultSet rs = consulta.executeQuery();
			if (rs.next()) {
				pessoaPesquisada = construirPessoaDoResultSet(rs);
			...
			System.out.println("Erro ao consultar pessoa por idade(idade: " + idade + ").\nCausa: " + e.getMessage());
		}
		return pessoaPesquisada;
	}
}
